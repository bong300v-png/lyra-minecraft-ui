...
function App() {
  ...
  useEffect(() => {
    fetch('https://api.mc-complex.com/servers')
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        // Normalize guide data (some have 'title', some have 'name')
        const normalizedData = data.map(server => ({
          ...server,
          guides: server.guides?.map(guide => ({
  ...
  const getPlayUrl = (server) => {
    if (server.name === 'Cobblemon') return server.forumsUrl + 'play/cobblemon';
    if (server.name === 'Pixelmon') return server.forumsUrl + 'play/pixelmon';
    return server.forumsUrl + 'play';
  };
      ...
      <>
        ...
        <div className="logo-wrap">
          <a href="index.html" className="logo">
            <img src="logo.png" alt="Complex Gaming Logo - Minecraft Server Network" />
          </a>
          {!loading && !error && (
            <span className="global-player-count">
              ...
              {servers.reduce((sum, s) => sum + s.gameModes.reduce((ms, m) => ms + m.players, 0), 0).toLocaleString()} online
            </span>
          )}
        </div>
        <nav className="right hidden-mobile" aria-label="Secondary navigation" style={{ visibility: 'hidden' }}>
          <a href="https://store.mc-complex.com/" className="nav-item store-button" role="presentation">
            Store
          ...
          <AnimatePresence mode="wait">
            {visibleServers.map((server, index) => (
              <motion.article
                ...
                style={{ '--primary-color': server.primaryColor }}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                transition={{
                ...
                <div className="server-content">
                  ...
                  >
                    <div className="player-count hidden-mobile">
                      ...
                      <span aria-label={`${server.gameModes.reduce((sum, mode) => sum + mode.players, 0)} players online`}>
                        {server.gameModes.reduce((sum, mode) => sum + mode.players, 0)} online
                      </span>
                    </div>
                    ...
                    <div className="server-actions">
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          window.open(getPlayUrl(server), '_blank');
                          // setExpandedServer(server.url);
                        }}
                  ...
                  {expandedServer === server.url && (
                    <>
                      ...
                      <div className="game-modes-list">
                        {server.gameModes.map((mode, modeIndex) => (
                      ...
                      {server.guides && server.guides.length > 0 && (
                        <>
                          ...
                          <div className="guides-list">
                            {server.guides.map((guide, guideIndex) => (
                              <a
                                key={guideIndex}
                                href={guide.url}
                                className="guide-item"
